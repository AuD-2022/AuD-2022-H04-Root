...
tasks {
    ...
    val graderJar by creating(Jar::class) {
        group = "build"
        afterEvaluate {
            archiveFileName.set("AuD-2022-H04-${project.version}.jar")
    ...
    val graderLibs by creating(Jar::class) {
        ...
        val runtimeDeps = grader.runtimeClasspath.mapNotNull {
            if (it.path.toLowerCase().contains("h04") || jagrRuntime.contains(it)) {
                null
        ...
        from(runtimeDeps)
        archiveFileName.set("AuD-2022-H04-${project.version}-libs.jar")
    }
    create("graderAll") {
        group = "build"
